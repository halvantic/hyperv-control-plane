...
func TestOnlyABarePercentageIsDecorated(t *testing.T) {
	for _, c := range []struct{ in, label, want string }{
		{"42", "live migration", "live migration 42%"},
		{"100", "live migration", "live migration 100%"},
		{"copied 50 GB of 50 GB (100%)", "copy", "copied 50 GB of 50 GB (100%)"},
		{"exporting HVNew01 to HVNEW06 (50 GB)", "copy", "exporting HVNew01 to HVNEW06 (50 GB)"},
		{"made a temporary share on HVNEW06", "copy", "made a temporary share on HVNEW06"},
		/* The label belongs to the caller, and a storage migration is not a live
		   migration. The VM does not move: its files move underneath it, and it
		   keeps running on the host it was already on. Ballast told an operator
		   watching a storage move on HVNEW06 that their VM was live-migrating. */
		{"51", "moving storage", "moving storage 51%"},
		// No label decorates nothing but the percent sign — better than naming
		// an operation this code does not know it is doing.
		{"51", "", "51%"},
	} {
		if got := progressNote(c.in, c.label); got != c.want {
			t.Errorf("progressNote(%q, %q) = %q, want %q", c.in, c.label, got, c.want)
		}
	}
}

/*
The percentage must come from the migration that is RUNNING, and from this VM's.

	Msvm_MigrationJob instances stay behind once they finish, holding
	PercentComplete 100 for ever. The loop sorted every instance on the host by
	percentage and took the first, so the newest completed migration won
	permanently: the console showed 100% from the moment a move began. Measured
	on HVNEW06 with Hyper-V Manager showing the same storage move at 51%.

	Asserted on the generated script because the loop only runs on a host. What
	is checked is the shape of the decision: filter to unfinished, prefer this
	VM's, then order by start time — never by percentage.
*/
func TestProgressReadsTheRunningJobNotTheFinishedOne(t *testing.T) {
	s := migrateWithProgress("Move-VMStorage -VMName $vm")

	for _, want := range []string{
		// Everything before Completed. A finished job cannot be picked at all.
		"[int]$_.JobState -lt 7",
		// This VM's, when more than one is under way.
		"ResultClassName Msvm_ComputerSystem",
		"-contains $vm",
		// Newest, not highest.
		"Sort-Object StartTime -Descending",
	} {
		if !strings.Contains(s, want) {
			t.Errorf("missing %q in the progress loop:\n%s", want, s)
		}
	}
	if strings.Contains(s, "Sort-Object PercentComplete -Descending") {
		t.Error("the loop still orders migration jobs by percentage, which is what let a " +
			"finished job at 100 outrank the one actually running")
	}
}
