...
func (s *Stub) RemoveCSV(_ context.Context, name string) (string, error) {
	return "removed volume " + name + " and the storage behind it (stub)", nil
}
...
func (s *Stub) EnsureNodeMaintenance(_ context.Context, _ string, intent MaintenanceIntent, _ bool) (Outcome, NodeMaintenanceState, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	if !s.ClusterExists {
