...
func resourcesToProto(r types.HostResources) *HostResources {
	...
	for _, v := range r.Volumes {
		out.Volumes = append(out.Volumes, &StorageVolume{Name: v.Name, Path: v.Path, SizeBytes: v.SizeBytes, UsedBytes: v.UsedBytes, Shared: v.Shared, Unlettered: v.Unlettered, DiskUniqueId: v.DiskUniqueID, SharesDiskWithOs: v.SharesDiskWithOS})
	}
...
func resourcesFromProto(r *HostResources) types.HostResources {
	...
	for _, v := range r.GetVolumes() {
		out.Volumes = append(out.Volumes, types.StorageVolume{Name: v.GetName(), Path: v.GetPath(), SizeBytes: v.GetSizeBytes(), UsedBytes: v.GetUsedBytes(), Shared: v.GetShared(), Unlettered: v.GetUnlettered(), DiskUniqueID: v.GetDiskUniqueId(), SharesDiskWithOS: v.GetSharesDiskWithOs()})
	}
